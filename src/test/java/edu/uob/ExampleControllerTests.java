...
class ExampleControllerTests {
  ...
  void testBasicMoveTaking() throws OXOMoveException {
    ...
    failedTestComment = "Cell a2 wasn't claimed by the second player";
    assertEquals(secondMovingPlayer, controller.gameModel.getCellOwner(0, 1), failedTestComment);

    sendCommandToController("a3");
    // Check that A2 (cell [0,2] on the board) is now "owned" by the first player
    failedTestComment = "Cell a3 wasn't claimed by the first player";
    assertEquals(firstMovingPlayer, controller.gameModel.getCellOwner(0, 2), failedTestComment);
    controller.reset();
  }
  ...
  void testBasicDraw() throws OXOMoveException {
    // Test draw on 3x3 board
    // Fill all cells but player 1 wins (no draw)
    sendCommandToController("a1"); // First player
    ...
    String failedTestComment = "Game should have been won, but it was drawn";
      assertFalse(model.isGameDrawn(), failedTestComment);
    controller.reset();

    // Fill all cells for draw
    sendCommandToController("a1"); // First player
    sendCommandToController("b2"); // Second player
    sendCommandToController("c3"); // First player
    sendCommandToController("c2"); // Second player
    ...
    failedTestComment = "Game should have been a draw, but it wasn't";
    assertTrue(model.isGameDrawn(), failedTestComment);

    //add row after draw
    //check it's no longer a draw
    controller.addRow();
    failedTestComment = "Game should not be drawn after added row, but it is";
    assertFalse(model.isGameDrawn(), failedTestComment);

    //remove row
    //check it's a draw again
    controller.removeRow();
    failedTestComment = "Game should drawn after row removal, but it isn't";
    assertTrue(model.isGameDrawn(), failedTestComment);

    //add column after draw
    //check it's no longer a draw
    controller.addColumn();
    failedTestComment = "Game should not be drawn after added column, but it is";
    assertFalse(model.isGameDrawn(), failedTestComment);

    //remove column
    //check it's a draw again
    controller.removeColumn();
    failedTestComment = "Game should drawn after column removal, but it isn't";
    assertTrue(model.isGameDrawn(), failedTestComment);
